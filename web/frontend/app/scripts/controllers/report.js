...
angular.module('newshoundApp')
	.controller('ReportCtrl', ['$scope', '$window', '$location', '$modal', '$filter', 'news', 'report',
		function($scope, $window, $location, $modal, $filter, news, report) {
			...
			var senders = report.getSenderReport();
			...
			// build report totals
			var totals = report.getTotalReport();
			totals.then(function(data) {
				$scope.totals = data;
			}, function(reason) {
				alert(reason);
			});
			...
			var addSenderLocation = function(id) {
				var searchVals = $location.search();
				searchVals.sender = id;
				$location.search(searchVals);
			};

			var clearSenderLocation = function() {
				$scope.senderName = undefined;
				$scope.senderDialog = undefined;
				var searchVals = $location.search();
				delete searchVals['sender'];
				$location.search(searchVals);
			};
